...
if (isset($_POST['register'])) {
    ...
    print_r($valid);
    if (!empty($valid)) {
        header('Location:../index.php?registor_error=1&user_name=' . $_POST['user_name'] . '&email=' . $_POST['email'] . '&country=' . $_POST['country'] . '&phone=' . $_POST['user_phone']."&mail_in=".$val->mail_in_use());
        echo "error";
        die();
    } else {
        echo "no error";
